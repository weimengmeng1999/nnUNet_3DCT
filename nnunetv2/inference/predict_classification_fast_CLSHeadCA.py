...
class nnUNetPredictorWithClassification(nnUNetPredictor):
    ...
    def _load_classification_head(self, model_folder: str, fold: Union[int, str]):
        ...
        #     nn.ReLU(),
        #     nn.Linear(128, self.mt_num_classes)
        # ).to(self.device) # harder LNorm experiment
    ...
    def _internal_maybe_mirror_and_predict_with_features(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        prediction = self.network.decoder(features)

        bottleneck_feature_0 = features[-3]
        bottleneck_feature_1 = features[-2]
        bottleneck_feature_2 = features[-1]
        ...
        return prediction, bottleneck_feature_0, bottleneck_feature_1, bottleneck_feature_2
        ...
        try:
            ...
            with tqdm(desc=None, total=len(slicers), disable=not self.allow_tqdm) as pbar:
                while True:
                    ...
                    prediction, features_0, features_1, features_2 = self._internal_maybe_mirror_and_predict_with_features(workon)
                    ...
                    # FIXED: Collect all features instead of overwriting
                    if features_0 is not None:
                        all_features_0.append(features_0.clone().to(results_device))  
                    if features_1 is not None:   
                        all_features_1.append(features_1.clone().to(results_device))
                    if features_2 is not None:
                        all_features_2.append(features_2.clone().to(results_device))
            ...
            if all_features_0:
                predicted_features_0 = torch.stack(all_features_0).mean(dim=0)
            else:
                predicted_features_0 = None
            if all_features_1:
                predicted_features_1 = torch.stack(all_features_1).mean(dim=0)
            else:
                predicted_features_1 = None
            if all_features_2:
                predicted_features_2 = torch.stack(all_features_2).mean(dim=0)
            else:
                predicted_features_2 = None
            ...
            del predicted_logits, n_predictions, prediction, gaussian, workon, all_features
            empty_cache(self.device)
            empty_cache(results_device)
            raise e
        
        return predicted_logits, predicted_features_0, predicted_features_1, predicted_features_2
    ...
    def predict_sliding_window_return_logits_and_features(self, input_image: torch.Tensor) \
        ...
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.perform_everything_on_device and self.device != 'cpu':
                try:
                    predicted_logits, predicted_features_0, predicted_features_1, predicted_features_2 = self._internal_predict_sliding_window_return_logits_and_features(data, slicers,
                                                                                           self.perform_everything_on_device)
                except RuntimeError:
                    print(
                        'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                    empty_cache(self.device)
                    predicted_logits, predicted_features_0, predicted_features_1, predicted_features_2 = self._internal_predict_sliding_window_return_logits_and_features(data, slicers, False)
            else:
                predicted_logits, predicted_features_0, predicted_features_1, predicted_features_2 = self._internal_predict_sliding_window_return_logits_and_features(data, slicers,
                                                                                       self.perform_everything_on_device)
            empty_cache(self.device)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i, params in enumerate(self.list_of_parameters):
            ...
            if prediction is None:
                predicted_logits, predicted_features_0, predicted_features_1, predicted_features_2 = self.predict_sliding_window_return_logits_and_features(data)
                print(f'Fold {i}: predicted_logits shape: {predicted_logits.size()}')
                print(f'Fold {i}: predicted_features_0 shape: {predicted_features_0.size() if predicted_features_0 is not None else None}')
                print(f'Fold {i}: predicted_features_1 shape: {predicted_features_1.size() if predicted_features_1 is not None else None}')
                print(f'Fold {i}: predicted_features_2 shape: {predicted_features_2.size() if predicted_features_2 is not None else None}')
                prediction = predicted_logits.to('cpu')
                features_0 = predicted_features_0.to('cpu') if predicted_features_0 is not None else None
                features_1 = predicted_features_1.to('cpu') if predicted_features_1 is not None else None
                features_2 = predicted_features_2.to('cpu') if predicted_features_2 is not None else None
            else:
                predicted_logits, predicted_features_0, predicted_features_1, predicted_features_2 = self.predict_sliding_window_return_logits_and_features(data)
                prediction += predicted_logits.to('cpu')
        ...
        print('Final features shape 0:', features_0.size() if features_0 is not None else None)
        print('Final features shape 1:', features_1.size() if features_1 is not None else None)
        print('Final features shape 2:', features_2.size() if features_2 is not None else None)

        if self.verbose: print('Prediction done')
        torch.set_num_threads(n_threads)
        return prediction, features_0, features_1, features_2   
    ...
    def predict_logits_and_features_from_preprocessed_data(self, data: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        seg_logits, encoder_features_0, encoder_features_1, encoder_features_2 = self.predict_logits_from_preprocessed_data(data)
        
        return seg_logits, encoder_features_0, encoder_features_1, encoder_features_2
    ...
    def predict_from_files_sequential(self,
        ...
        for case_idx, (li, of, sps) in enumerate(zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files)):
            ...
            seg_logits, encoder_features_0, encoder_features_1, encoder_features_2 = self.predict_logits_and_features_from_preprocessed_data(
                torch.from_numpy(data)
            )

            bottleneck_feature_0 = encoder_features_0.to(self.device).float()
            bottleneck_feature_1 = encoder_features_1.to(self.device).float()
            bottleneck_feature_2 = encoder_features_2.to(self.device).float()


            
            # Use encoder features for classification
            cls_results = None
            if self.cls_head is not None and encoder_features_0 is not None:
                case_name = os.path.basename(of) if of else f"case_{case_idx}"
                print(f"Case: {case_name}")
                print(f"  Encoder features shape: {encoder_features_0.shape}")
                print(f"  Encoder features mean: {encoder_features_0.mean().item():.6f}")
                print(f"  Encoder features std: {encoder_features_0.std().item():.6f}")
                print(f"  Encoder features min: {encoder_features_0.min().item():.6f}")
                print(f"  Encoder features max: {encoder_features_0.max().item():.6f}")
                
                with torch.no_grad():
                    # Move to device and ensure float precision
                    # pooled = encoder_features_0.to(self.device).float()
