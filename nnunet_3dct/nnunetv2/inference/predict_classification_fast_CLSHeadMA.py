...
class Classifier3D(nn.Module):
    def __init__(
        ...
        """
        Args:
            in_channels_list: list[int] channel dims of incoming 3D feature maps
            H,W,D: fixed spatial size to which each feature is trilinearly resized
            d: hidden dim after linear projection
            N: number of learnable memory tokens (paper uses N = H*W*D = 200)
            num_layers: number of (cross+self)-attention + MLP blocks (paper mentions four-level)
            num_heads: attention heads
            mlp_ratio: expansion in FFN
            dropout: dropout prob in attn and MLP
        ...
        # 1x1x1 projections per feature map to d
        self.proj_layers = nn.ModuleList([nn.Conv3d(c, d, kernel_size=1) for c in in_channels_list])

        # Shared learnable positional embedding for the resized feature tokens (N_hat, d)
        self.xpos = nn.Parameter(torch.zeros(self.N_hat, d))
        nn.init.trunc_normal_(self.xpos, std=0.02)

        # Learnable global memory (N, d)
        self.memory = nn.Parameter(torch.randn(N, d) * 0.02)

        # Linear projections:
        # memory: q, k, v  (each d->d)
        self.mem_q = nn.Linear(d, d)
        self.mem_k = nn.Linear(d, d)
        self.mem_v = nn.Linear(d, d)

        # content (flattened features): k, v  (each d->d)
        self.content_k = nn.Linear(d, d)
        self.content_v = nn.Linear(d, d)

        # Stacks of (Attn -> Add&Norm -> MLP -> Add&Norm)
        self.layers = nn.ModuleList([
            _MemBlock(d, num_heads, mlp_ratio, dropout)
            for _ in range(num_layers)
        ])

        # Classifier on mean-pooled memory
    ...
    def _prep_tokens(self, *features):
        """
        Resize, project, flatten, add positional embedding.
        Input features: list of (B, C, D, H, W)
        Returns:
            content_tokens: (B, L, d) where L = K * N_hat (K = #features)
        """
        tokens = []
        for feat, proj in zip(features, self.proj_layers):
            # feat: (B, C, D, H, W) -> resize to (B, C, D', H', W') with trilinear
            f = F.interpolate(feat, size=(self.D, self.H, self.W), mode='trilinear', align_corners=False)
            f = proj(f)  # (B, d, D', H', W')
            # flatten to (B, N_hat, d)
            f = f.flatten(2).transpose(1, 2)
            # add shared learnable positional embedding (broadcast over batch)
            f = f + self.xpos[None, :, :]
            tokens.append(f)
        content_tokens = torch.cat(tokens, dim=1)  # (B, K*N_hat, d)
        return content_tokens

    def _qkv(self, memory, content):
        """
        memory: (B, N, d)
        content: (B, L, d)
        Returns:
            ...
            v_cm: (B, L+N, d)
        """
        q_m = self.mem_q(memory)
        k_m = self.mem_k(memory)
        v_m = self.mem_v(memory)

        k_c = self.content_k(content)
        v_c = self.content_v(content)

        k_cm = torch.cat([k_c, k_m], dim=1)
        v_cm = torch.cat([v_c, v_m], dim=1)
        return q_m, k_cm, v_cm

    def forward(self, *features):
        """
        *features: variable number of 3D feature maps, each (B, C, D, H, W)
        """
        assert len(features) == len(self.proj_layers), "Number of features must match in_channels_list length"
        B = features[0].size(0)

        # Prepare content tokens
        content = self._prep_tokens(*features)  # (B, L, d)

        # Expand memory to batch: (B, N, d)
        mem = self.memory.unsqueeze(0).expand(B, -1, -1)

        # Stacked cross+self attention blocks with concatenated K/V
        for layer in self.layers:
            q_m, k_cm, v_cm = self._qkv(mem, content)
            mem = layer(mem, q_m, k_cm, v_cm)  # (B, N, d)

        # Mean-pool memory and classify
        pooled = mem.mean(dim=1)  # (B, d)
        return self.classifier(pooled)  # (B, num_classes)


class _MemBlock(nn.Module):
    """One block implementing: attention(y) = softmax(q_m k_cm^T / sqrt(d)) v_cm, then MLP; each with Add&Norm."""
    def __init__(self, d, num_heads, mlp_ratio, dropout):
        super().__init__()
        self.d = d
        self.h = num_heads
        self.head_dim = d // num_heads
    ...
    def _split_heads(self, x):
        # x: (B, T, d) -> (B, h, T, head_dim)
        B, T, d = x.shape
        x = x.view(B, T, self.h, self.head_dim).transpose(1, 2)
        return x

    def _merge_heads(self, x):
        # x: (B, h, T, head_dim) -> (B, T, d)
        B, h, T, hd = x.shape
        return x.transpose(1, 2).contiguous().view(B, T, h * hd)

    def forward(self, mem_in, q_m, k_cm, v_cm):
        """
        mem_in: (B, N, d) residual path
        q_m:    (B, N, d)
        k_cm:   (B, L+N, d)
        v_cm:   (B, L+N, d)
        """
        # Multi-head attention on memory queries over concatenated keys/values
        q = self._split_heads(q_m)          # (B, h, N, hd)
        k = self._split_heads(k_cm)         # (B, h, L+N, hd)
        v = self._split_heads(v_cm)         # (B, h, L+N, hd)

        attn = torch.matmul(q, k.transpose(-2, -1)) * self.scale  # (B, h, N, L+N)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        y = torch.matmul(attn, v)                                  # (B, h, N, hd)
        y = self._merge_heads(y)                                   # (B, N, d)
        y = self.proj(y)
        y = self.proj_drop(y)

        # Add & Norm
        y = self.norm1(mem_in + y)
...
class nnUNetPredictorWithClassification(nnUNetPredictor):
    ...
    def _load_classification_head(self, model_folder: str, fold: Union[int, str]):
        ...
        self.cls_head = Classifier3D(in_channels_list=[128, 320, 320, 128],
                                    H=5, W=8, D=5,
                                    d=320,
                                    N=128,
                                    num_layers=4,
    ...
    def _internal_maybe_mirror_and_predict_with_features(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        cls_logits = self.cls_head(encoder_features[2], encoder_features[4], decoder_features[0], decoder_features[2])
        # for ver 12
    ...
    def predict_from_files_sequential(self,
        ...
        for case_idx, (li, of, sps) in enumerate(zip(list_of_lists_or_source_folder,
            ...
            csv_results.append([case_name + ".nii.gz", pred_label])

            ret.append(cls_results)
        ...
        with open(join(output_folder, 'subtype_results.csv'), "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Names", "Subtype"])  # header
...
def predict_entry_point_with_classification():
    ...
    parser.add_argument('-i', type=str, required=False, default= "/home/mengwei/Downloads/pancreas-segmentation-classification/data_ver2/nnUNet_raw/Dataset001_3DCT/imagesTs",
                       help='input folder. Remember to use the correct channel numberings for your files (_0000 etc). '
                             'File endings must be the same as the training dataset!')
    parser.add_argument('-o', type=str, required=False, default = "/home/mengwei/Downloads/nnUNet_code/pretrain/test_re",
                       help='Output folder. If it does not exist it will be created. Predicted segmentations will '
                             'have the same name as their source images.')
    parser.add_argument('-co', '--cls_output_folder', type=str, default="/home/mengwei/Downloads/nnUNet_code/pretrain/test_re",
    ...
    parser.add_argument('-chk', type=str, required=False, default='checkpoint_cls_best.pth',
