...
class nnUNetPredictorWithClassification(nnUNetPredictor):
    ...
    def _internal_predict_sliding_window_return_logits_and_features(self,
        ...
        try:
            ...
            with tqdm(desc=None, total=len(slicers), disable=not self.allow_tqdm) as pbar:
                while True:
                    ...
                    prediction, features = self._internal_maybe_mirror_and_predict_with_features(workon)
                    ...
                    if features is not None:
                        all_features.append(features.clone().to(results_device))
            ...
            if all_features:
                predicted_features = torch.stack(all_features).mean(dim=0)
            else:
        ...
        return predicted_logits, predicted_features
    ...
    def predict_sliding_window_return_logits_and_features(self, input_image: torch.Tensor) \
        ...
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.perform_everything_on_device and self.device != 'cpu':
                try:
                    predicted_logits, predicted_features = self._internal_predict_sliding_window_return_logits_and_features(data, slicers,
                                                                                           self.perform_everything_on_device)
                except RuntimeError:
                    print(
                        'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                    empty_cache(self.device)
                    predicted_logits, predicted_features = self._internal_predict_sliding_window_return_logits_and_features(data, slicers, False)
            else:
                predicted_logits, predicted_features = self._internal_predict_sliding_window_return_logits_and_features(data, slicers,
                                                                                       self.perform_everything_on_device)
            empty_cache(self.device)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i, params in enumerate(self.list_of_parameters):
            ...
            if prediction is None:
                predicted_logits, predicted_features = self.predict_sliding_window_return_logits_and_features(data)
                print(f'Fold {i}: predicted_logits shape: {predicted_logits.size()}')
                print(f'Fold {i}: predicted_features shape: {predicted_features.size() if predicted_features is not None else None}')
                prediction = predicted_logits.to('cpu')
                features = predicted_features.to('cpu') if predicted_features is not None else None
            else:
                predicted_logits, predicted_features = self.predict_sliding_window_return_logits_and_features(data)
                prediction += predicted_logits.to('cpu')
        ...
        print('Final features shape:', features.size() if features is not None else None)
    ...
    def predict_logits_and_features_from_preprocessed_data(self, data: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        seg_logits, encoder_features = self.predict_logits_from_preprocessed_data(data)
        
        return seg_logits, encoder_features
    ...
    def predict_from_files_sequential(self,
        ...
        for case_idx, (li, of, sps) in enumerate(zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files)):
            ...
            seg_logits, encoder_features = self.predict_logits_and_features_from_preprocessed_data(
                torch.from_numpy(data)
            )
            
            # Use encoder features for classification
            cls_results = None
            if self.cls_head is not None and encoder_features is not None:
                case_name = os.path.basename(of) if of else f"case_{case_idx}"
                print(f"Case: {case_name}")
                print(f"  Encoder features shape: {encoder_features.shape}")
                print(f"  Encoder features mean: {encoder_features.mean().item():.6f}")
                print(f"  Encoder features std: {encoder_features.std().item():.6f}")
                print(f"  Encoder features min: {encoder_features.min().item():.6f}")
                print(f"  Encoder features max: {encoder_features.max().item():.6f}")
