...
class nnUNetDataLoader(DataLoader):
    def __init__(self,
        ...
        labels_file = os.path.abspath(join(self._data.source_folder, os.pardir, 'classification.json'))
        self.classification_label_mapping = load_json(labels_file) if os.path.isfile(labels_file) else None
    ...
    def generate_train_batch(self):
        ...
        cls_target_np = None
        if self.classification_label_mapping is not None:
            labels = []
            for k in selected_keys:
                if k not in self.classification_label_mapping:
                    raise KeyError(f"classification.json missing entry for case: {k}")
                v = self.classification_label_mapping[k]
                labels.append(int(v) if isinstance(v, str) else v)
            cls_target_np = np.asarray(labels, dtype=np.int64)
        ...
        if self.transforms is not None:
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    ...
                    del segs, images
            out = {'data': data_all, 'target': seg_all, 'keys': selected_keys}
            if cls_target_np is not None:
                out['cls_target'] = torch.from_numpy(cls_target_np).long()
            return out
...
if __name__ == '__main__':
    ...
    print("Batch keys:", a.keys())       # if a is a dict-like object
    print("Batch data shapes:")
    for k, v in a.items():
        print(f"  {k}: {v.shape if hasattr(v, 'shape') else type(v)}")
    print("Batch sample data (example):", {k: (v[0] if hasattr(v, '__getitem__') else v) for k, v in a.items()})
