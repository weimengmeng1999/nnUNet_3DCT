...
class nnUNetLogger(object):
    ...
    def plot_progress_png_ocls(self, output_folder):
        # only consider the mandatory keys for epoch length
        # epoch = min([len(i) for i in self.my_fantastic_logging.values()]) - 1
        epoch = min([len(self.my_fantastic_logging[k]) 
             for k in ['lrs', 'epoch_start_timestamps', 'epoch_end_timestamps', 'val_classification_acc', 'val_classification_f1']]) - 1
        sns.set(font_scale=2.5)
        fig, ax_all = plt.subplots(4, 1, figsize=(30, 72))  # w only classification f1 score
        ...
        ax.plot(x_values, self.my_fantastic_logging['train_losses'][:epoch + 1], color='b', ls='-', label="loss_tr", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_losses'][:epoch + 1], color='r', ls='-', label="loss_val", linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['val_classification_f1'][:epoch + 1], color='g', ls='-', label="pseudo macro f1",linewidth=3)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
    ...
    def plot_progress_png_wcls(self, output_folder):
        # only consider the mandatory keys for epoch length
        # epoch = min([len(i) for i in self.my_fantastic_logging.values()]) - 1
        epoch = min([len(self.my_fantastic_logging[k]) 
             for k in ['mean_fg_dice', 'ema_fg_dice',
             'lrs', 'epoch_start_timestamps', 'epoch_end_timestamps', 'val_classification_acc', 'val_classification_f1',
        ...
        ax2.plot(x_values, self.my_fantastic_logging['ema_fg_dice'][:epoch + 1], color='g', ls='-', label="pseudo dice (mov. avg.)",
                 linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['val_classification_f1'][:epoch + 1], color='y', ls='-', label="pseudo macro f1",linewidth=3)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
        ax2.set_ylabel("dice/macro f1")
        ax.legend(loc=(0, 1))
        ax2.legend(loc=(0.2, 1))
